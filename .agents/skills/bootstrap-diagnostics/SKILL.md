...
description: >-
  ...
  Use whenever the session-start digest's bootstrap or network-checks section prints an actionable diagnostic line - MISSING, MISSING_MANUAL, PRESENTATION_UNAVAILABLE, BACKEND_INVALID, NEEDS_GH_AUTH, TANGLE, STARTUP_MEMORY_BUDGET, CREW_DISPATCH invalid, FLEET_SYNC, NETWORK_CHECKS, HOME_SUMMARY, BACKLOG_RECONCILE, SECONDMATE_SYNC, SECONDMATE_LIVENESS, SECONDMATE_HANDOFF, NUDGE_SECONDMATES, PRESERVATION, or FMX - or reports that an interrupted backlog cleanup may have left an endpoint or local copy, or when a standalone bin/fm-bootstrap.sh or bin/fm-startup-network.sh run prints one of those lines.
...
- `PRESERVATION: <n> in-flight tasks without a current receipt` - a network-free advisory scan (`bin/fm-preservation-staleness-lib.sh`) found ship or scout tasks whose newest AgentLab evidence-preservation checkpoint receipt is missing or stale against their own current work (`docs/evidence-preservation-lifecycle.md` in `yagakeerthikiran/agentlab-shared-memory` is the canonical contract).
  Identify the specific tasks from the fleet-state digest's own per-task `preservation:` line (`bin/fm-session-start.sh`) rather than re-deriving the count, and publish and record an updated checkpoint for each rather than waiting for a spawn, promotion, teardown, or merge to refuse on it.
